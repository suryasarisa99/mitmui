...
import 'package:mitmui/widgets/preview_response_body.dart';
...
class _ResponsePanelState extends State<ResponsePanel>
    with SingleTickerProviderStateMixin {
  late TabController _tabController;
  Future<MitmBody>? _responseBodyFuture;
  Future<dynamic>? _responseDataFuture;

  @override
  void initState() {
    super.initState();
    _tabController = TabController(length: 3, vsync: this);
    _responseBodyFuture = MitmproxyClient().getMitmBody(
      widget.flow!.id,
      'response',
    );
    _responseDataFuture = MitmproxyClient().getMitmContent(
      widget.flow!.id,
      'response',
    );
  }
          ...
          Expanded(
            child: TabBarView(
              ...
              children: [
                ...
                // Body Tab
                _buildBodyView(response, _responseBodyFuture),

                // Preview Tab
                PreviewResponseBody(
                  response: response,
                  responseBodyFuture: _responseBodyFuture,
                  responseDataFuture: _responseDataFuture,
  ...
  Widget _buildBodyView(
    ...
    Future<MitmBody>? responseBodyFuture,
  ) {
    final hasContent =
    ...
    return FutureBuilder<MitmBody>(
      ...
      builder: (context, snapshot) {
        ...
        } else if (!snapshot.hasData || snapshot.data!.text.isEmpty) {
